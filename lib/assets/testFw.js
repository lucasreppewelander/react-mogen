...
const installHelperPackages = async (framework, pkg) => {
	...
	const reactPackageVersion = await packages.find(pk => {
	...
	if (framework === 'enzyme') {
		await console.log(colors.ok(`npm install jest jsdom enzyme-adapter-react-${reactPackageVersion} --save-dev`));
		await exec(`npm install jest jsdom enzyme-adapter-react-${reactPackageVersion} --save-dev`, { stdio: [0,1,2] });
	}

	return reactPackageVersion;
};
